...
def test_create_decision_session_expected(auth_header):
    ...
    response = client.post("/api/v1/decisions/sessions", json={"title": "Test Session"}, headers=auth_header)
...
def test_create_decision_session_edge(auth_header):
    ...
    response = client.post("/api/v1/decisions/sessions", json={"title": ""}, headers=auth_header)
...
def test_create_message_expected(auth_header):
    ...
    if not sid:
        resp = client.post("/api/v1/decisions/sessions", json={"title": "Msg Session"}, headers=auth_header)
        sid = resp.json()["id"]
    payload = {"content": "Hello AI!", "sender": "user"}
    response = client.post(f"/api/v1/decisions/sessions/{sid}/messages", json=payload, headers=auth_header)
    assert response.status_code == 201
    data = response.json()
...
def test_create_message_failure(auth_header):
    ...
    response = client.post(f"/api/v1/decisions/sessions/00000000-0000-0000-0000-000000000000/messages", json=payload, headers=auth_header)
...
def test_list_messages_expected(auth_header):
    ...
    sid = globals().get("session_id")
    response = client.get(f"/api/v1/decisions/sessions/{sid}/messages", headers=auth_header)
...
def test_list_messages_failure(auth_header):
    ...
    response = client.get(f"/api/v1/decisions/sessions/00000000-0000-0000-0000-000000000000/messages", headers=auth_header)
    assert response.status_code == 404
...
def test_update_session_expected(auth_header):
    ...
    response = client.patch(f"/api/v1/decisions/sessions/{sid}", json=payload, headers=auth_header)
    assert response.status_code == 200
    data = response.json()
...
def test_update_session_failure(auth_header):
    ...
    response = client.patch(f"/api/v1/decisions/sessions/00000000-0000-0000-0000-000000000000", json=payload, headers=auth_header)
    assert response.status_code == 404
